...
def train_model(config: TrainConfig) -> dict[str, object]:
    ...
    model = MnistConvNet(channels=config.channels, dropout=config.dropout).to(device)
...
def evaluate_checkpoint(config: TrainConfig) -> dict[str, object]:
    ...
    model = MnistConvNet(channels=int(checkpoint["config"]["channels"]), dropout=float(checkpoint["config"]["dropout"])).to(device)
