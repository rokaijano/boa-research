...
from .schema import CandidateMetadata, IncumbentRecord, PatchDescriptor, StageRunResult, TrialSummary
...
class ExperimentStore:
        ...
        with self.connect() as conn:
            conn.execute(
                ...
                INSERT OR REPLACE INTO trials (
                    ...
                    acceptance_status, canonical_stage, canonical_score, candidate_json,
                    descriptor_json, search_decision_json, diff_path, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, NULL, NULL, ?, ?, ?, ?, ?, ?)
                """,
                (
                    trial_id,
                    ...
                    json.dumps(search_decision, default=_json_default, sort_keys=True),
    ...
    def recent_trials(self, *, limit: int = 20) -> list[TrialSummary]:
        with self.connect() as conn:
            rows = conn.execute(
                """
                SELECT trial_id, branch_name, parent_branch, parent_trial_id, acceptance_status,
                       canonical_stage, canonical_score, candidate_json, descriptor_json, created_at, updated_at
                FROM trials
