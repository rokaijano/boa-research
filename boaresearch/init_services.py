...
def _analysis_prompt(repo_root: Path, selection: InitSetupSelection) -> str:
    ...
    sections = [
        ...
        f"Selected agent preset: {selection.agent_preset}",
        f"Selected runner mode: {selection.runner_mode}",
        "Repository tree:",
        _tree_listing(repo_root),
    ...
    sections.extend(
        [
            ...
            json.dumps(REPO_ANALYSIS_SCHEMA, indent=2, sort_keys=True),
        ]
    )
    return "\n\n".join(sections)
...
def build_config_from_plan(plan: ReviewedInitPlan) -> BoaConfig:
    ...
    return BoaConfig(
        ...
        guardrails=GuardrailConfig(
            allowed_paths=list(plan.editable_files),
            protected_paths=list(plan.protected_files),
        ),
        runner=build_runner_config(selection, scout_commands=[plan.train_command, plan.eval_command]),
        metrics=[metric],
        objective=ObjectiveConfig(
            primary_metric=plan.primary_metric_name,
            direction=plan.metric_direction,
        ),
        search=SearchConfig(policy="local_ranking"),
...
def render_config_text(plan: ReviewedInitPlan) -> str:
    ...
    lines.extend(
        [
            ...
            f"fallback_token_env = {_quote(config.git_auth.fallback_token_env or '')}",
    ...
    if config.runner.ssh.key_path:
        lines.append(f"key_path = {_quote(str(config.runner.ssh.key_path))}")
    ...
    lines.extend(
        [
            ...
            "",
            "[search]",
            f"policy = {_quote(config.search.policy)}",
...
def render_boa_md(plan: ReviewedInitPlan) -> str:
    ...
    return "\n".join(
        [
            ...
            "## BOA Setup Summary",
            "",
            f"- Train command: `{plan.train_command}`",
            f"- Eval command: `{plan.eval_command}`",
            f"- Primary metric: `{plan.primary_metric_name}` ({plan.metric_direction})",
            f"- Editable paths: {', '.join(plan.editable_files)}",
            f"- Protected paths: {', '.join(plan.protected_files)}",
            f"- Likely optimization surfaces: {surfaces}",
            "",
            "## Caveats",
            "",
            caveats,
